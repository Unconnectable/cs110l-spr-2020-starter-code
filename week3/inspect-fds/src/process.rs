...
mod test {
    ...
    fn test_list_fds() {
        ...
        /*原来的测试有误 cargo本身会打开一些FD 也就是下面的 只需要检查是否含有杰克
        assertion `left == right` failed
        left: [0, 1, 2, 4, 5, 22, 24, 28]
        right: [0, 1, 2, 4, 5]
         */
        let actual_fds = process.list_fds().expect("Expected list_fds to find file descriptors");

        // 检查实际的 FD 列表是否包含所有预期的 FD
        assert!(actual_fds.contains(&0));
        assert!(actual_fds.contains(&1));
        assert!(actual_fds.contains(&2));
        assert!(actual_fds.contains(&4));
        assert!(actual_fds.contains(&5));
        // assert_eq!(
        //     process
        //         .list_fds()
