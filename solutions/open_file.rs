...
impl OpenFile {
    ...
    fn parse_cursor(fdinfo: &str) -> Option<usize> {
        ...
        Some(re.captures(fdinfo)?.get(1)?.as_str().parse::<usize>().ok()?)
    }
...
mod test {
    ...
    fn test_openfile_from_fd() {
        ...
        let open_file = OpenFile::from_fd(process.pid, 0).expect(
            "Expected to get open file data for multi_pipe_test, but OpenFile::from_fd returned None"
        );
